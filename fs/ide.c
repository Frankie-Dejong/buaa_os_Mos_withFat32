...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	u_int op = DEV_DISK_OPERATION_READ;
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_write_dev(&offset, DEV_DISK_ADDRESS + DEV_DISK_OFFSET, sizeof(uint32_t)));
		panic_on(syscall_write_dev(&op, DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, sizeof(uint32_t)));
		panic_on(syscall_read_dev(&success, DEV_DISK_ADDRESS + DEV_DISK_STATUS, sizeof(uint32_t)));
		if(!success) {
			user_panic("ide_read fail: success is zero\n");
		}else {
...
void ide_write(u_int diskno, u_int secno, void *src, u_int nsecs) {
	u_int begin = secno * BY2SECT;
	u_int end = begin + nsecs * BY2SECT;
	u_int op = DEV_DISK_OPERATION_WRITE;
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_read_dev(&success, DEV_DISK_ADDRESS + DEV_DISK_STATUS, 4));
		if(!success) {
			user_panic("ide_write fail: success is zero\n");
		}
	}
}
