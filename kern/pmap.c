...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	*ppte = KADDR((PPN(*pgdir_entryp) << 12)) + (PTX(va)<<2); 
	return 0;
}
...
//		if(index >= 16)
//			index = 0;
//		u_long curPa  = SWAP_PAGE_BASE | (index << 12);
//		index ++;
...
//				{
//					 Pte *pte  = KADDR((PPN(*pgdir_entry) << 12)) + (j << 2);
//					 if((*pte) & PTE_V)
//					 {
//						 if((*pte) >> 12 == curPa >> 12)
//						 {
...
//							 tlb_invalidate(asid, va);
//							 memcpy(da, va, BY2PG);
//							 LIST_INSERT_HEAD(&page_free_swapable_list, pa2page(curPa), pp_link);
//						 }
//					 }
//					 else
//						 continue;
