...
static inline int is_illegal_pa_range(u_long pa, u_int len) {
	...
	if(pa >= 0x13000000U && pa + len <= 0x13004200U) {
		return 0;
	}
	if(pa >= 0x15000000U && pa + len <= 0x15000200U) {
		return 0;
	}
	return 1;
}
...
int sys_ipc_try_broadcast(struct Env * tenvs, u_int val, void * srcva, u_int perm) {
	static u_int sons[20] = {};
	int top = -1;
	struct Env *e;
	for(int i = 0;i < NENV;i ++) {
		if(curenv->env_id == tenvs[i].env_parent_id) {
			sons[++top] = tenvs[i].env_id;
		}
	}
	if(top == -1) {
		return 0;
	...
	for(int i = 0;i <= top;i ++) {
		envid2env(sons[i], &e, 0);
		for(int j = 0;j < NENV;j ++) {
			if(e->env_id == tenvs[j].env_parent_id) {
				sons[++top] = tenvs[j].env_id;
			}
		}
	}
	for(int i = 0;i <= top;i ++) {
	...
	for(int i = 0;i <= top;i ++) {
		try(sys_ipc_try_send(sons[i], val, srcva, perm));
	}
	return 0;
}
