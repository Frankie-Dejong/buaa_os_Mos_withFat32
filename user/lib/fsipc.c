...
// open a fat32 file
int fsipc_open_Fat32(const char *path, u_int omode, struct Fd *fd) {
	u_int perm;
	struct Fsreq_open *req;
	...
	return fsipc(FSREQ_FAT | FSREQ_OPEN, req, fd, &perm);
}
...
//map a FAT32 file
int fsipc_map_Fat32(u_int fileid, u_int offset, void *dstva) {
	int r;
	u_int perm;
	struct Fsreq_map *req;
	...
	if ((r = fsipc(FSREQ_FAT | FSREQ_MAP, req, dstva, &perm)) < 0) {
		return r;
	}
...
int fsipc_set_size_Fat32(u_int fileid, u_int size) {
	struct Fsreq_set_size *req;

	req = (struct Fsreq_set_size *)fsipcbuf;
	req->req_fileid = fileid;
	req->req_size = size;
	return fsipc(FSREQ_FAT | FSREQ_SET_SIZE, req, 0, 0);
}
...
int fsipc_close_Fat32(u_int fileid) {
	struct Fsreq_close *req;

	req = (struct Fsreq_close *)fsipcbuf;
	req->req_fileid = fileid;
	return fsipc(FSREQ_FAT | FSREQ_CLOSE, req, 0, 0);
}
...
int fsipc_dirty_Fat32(u_int fileid, u_int offset) {
	struct Fsreq_dirty *req;
	req = (struct Fsreq_dirty *)fsipcbuf;
	req->req_fileid = fileid;
	req->req_offset = offset;
	return fsipc(FSREQ_FAT | FSREQ_DIRTY, req, 0, 0);
}
...
int fsipc_remove_Fat32(const char *path) {
	if(strlen(path) == 0 || strlen(path) > MAXPATHLEN) {
		return -E_BAD_PATH;
	}
	struct Fsreq_remove *req = (struct Fsreq_remove *)fsipcbuf;
...
int fsipc_flush_Fat32(u_int fileid, u_int offset, u_int n, void *src) {
	struct Fsreq_flush_Fat32 *req = (struct Fsreq_flush_Fat32 *)fsipcbuf;
	req->req_fileid = fileid;
	req->req_len = n;
	req->req_offset = offset;
