...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if(!(perm & PTE_COW)) {
		user_panic("%x is not a copy-on-write page\n", va);
	}
	...
	if(syscall_mem_alloc(0, UCOW, perm)) {
		user_panic("fail to allocate a new page at UCOW\n");
	}
	...
	if(syscall_mem_map(0, UCOW, 0, va, perm)) {
		user_panic("fail to map the page at UCOW to va with the new perm\n");
	}
	// Step 6: Unmap the page at 'UCOW'.
	/* Exercise 4.13: Your code here. (6/6) */
	if(syscall_mem_unmap(0, UCOW)) {
		user_panic("fail to unmap the page at UCOW\n");
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	if(!(perm & PTE_D)) {
		...
		if(r) {
			user_panic("fail on unWritePage\n");
		}
	}else if((perm & PTE_COW)) {
		r = syscall_mem_map(0, addr, envid, addr, perm);
		if(r) {
			user_panic("fail on PTE_COW\n");
		}
	}else if((perm & PTE_LIBRARY)) {
		r = syscall_mem_map(0, addr, envid, addr, perm);
		if(r) {
			user_panic("fail on PTE_LIBRARY\n");
		}
	}else {
		perm = perm | PTE_COW;
		perm = perm ^ PTE_D;
		r = syscall_mem_map(0, addr, envid, addr, perm);
		if(r) {
			user_panic("fail on map child\n");
		}
		r = syscall_mem_map(0, addr, 0, addr, perm);
		if(r) {
			user_panic("fail on map curenv\n");
		}
	}
}
...
int fork(void) {
	...
	for(int i = 0;i < USTACKTOP;i += BY2PG) {
		if((vpt[VPN(i)] & PTE_V) && (vpd[(i >> 22)] & PTE_V)) {
			duppage(child, VPN(i));
		}
	}
